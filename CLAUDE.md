...
- **Покроково.** Один логічний крок → показати написане → зупинитись. Не котити далі без паузи.
- **Не комітити самостійно.** `git add` / `commit` / `push` — тільки за прямим запитом
  користувача. До того зміни просто лежать у робочій директорії.
